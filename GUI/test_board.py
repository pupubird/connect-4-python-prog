...
class GameBoardPage:
    ...
    def _board(self, board_window, box_size):
        ...
        while True:
            ...
            if self.total_attempt == 3:
                self.save_data('board_data', board.data())
            self.window.refresh()

            # win check
            self.save_data('temp_board_data', board.data())
    ...
    def save_data(self, filename, content_list):
        import json
        ...
        with open(file, 'r') as f:
            ...
            with open(file, 'w') as g:
                ...
                data['board_data'][self.game_mode] = content_list
                data['meta'][self.game_mode]['total_attempt'] = self.total_attempt
                data['meta'][self.game_mode]['exists'] = 1
    ...
    def load_saved_data(self):
        ...
        with open('./assets/data/board_data.json', 'r') as f:
            data = json.load(f)
            return data['board_data'][self.game_mode], data['meta'][self.game_mode]['total_attempt']
